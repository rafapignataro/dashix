import { Avatar, Box, Flex, Grid, GridItem, HStack, Icon, Link, Select, SimpleGrid, Skeleton, Tab, TabList, TabPanel, TabPanels, Tabs, Text, VStack } from "@chakra-ui/react"
...
export const Home = () => {
  ...
  useEffect(() => { setTimeout(() => setFakeLoading(false), 500) }, [])
    ...
    <AppLayout title={`Bem vindo, ${session?.user.name?.split(' ')[0]}`}>
      <Grid templateColumns="1fr 1fr 1fr" templateRows="1fr 1fr" gap="4" h="100%">
        <GridItem colSpan={2} rowSpan={1}>
          {fakeLoading && <Skeleton height="100%" />}
          {!fakeLoading && (
            <Tabs 
              flex="1" 
